...
while($row = mysqli_fetch_array($raw_data)){
    ...
    $qty = 0;
        ...
        if($qty && $qty > 0){
            ...
            $re1 = mysqli_query($con3,"select sum(commission_amt) as commision_amount from order_detail where item_id='".$prcode."' and 
           ...
           $sqlimg="SELECT img_name FROM `product_images_new` WHERE `product_id`='".$product_id."'";
            ...
            $rowimg = mysqli_fetch_row($qryimg);
             if($youtube){
                ...
                $imgframe = '<img class="lazyload img-fluid product_img" loading="lazy" style="width: 100%; object-fit: contain; user-select: auto;" src="'.$_new_filename.'" alt="'.$product_name.'">';
            }
...
if(isset($row['sales_price']) && $row['sales_price'] > 0){
 ...
 ?>
    ...
    <div class="col-sm-3 product_grid" style="margin: 10px auto;" data-selling_price = <?= $lastSellingPrice ;?> data-rent_price = <?= $addedRentPrice ;?> >
        <img src="<?= ($_new_filename) ;  ?>" style="width:100%;" />
