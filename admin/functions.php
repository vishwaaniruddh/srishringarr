...
function get_product_from_cart_by_cartid($cartid){
    ...
    $sql=mysqli_query($con,"select product_id from cart where cart_id='".$cartid."'");
...
function get_product_type_from_cart($cartid,$userid){
    ...
    $sql=mysqli_query($con,"select * from cart where cart_id='".$cartid."' and user_id='".$userid."'");
...
function getDBrent($sku,$type){
    ...
    if($type==1){
        $sql ="select rent_price from product where product_code='".$sku."'";
    }else if($type==2){
        $sql ="select rent_price from garment_product where gproduct_code='".$sku."'";
    }
...
function get_order_ent($id,$parameter){
        ...
        $sql = mysqli_query($con,"select $parameter from Order_ent where id='".$id."'");
        $sql_result = mysqli_fetch_assoc($sql);
...
function get_order_details($id,$parameter){
        global $con;
        $sql = mysqli_query($con,"select $parameter from order_details where id='".$id."'");
        $sql_result = mysqli_fetch_assoc($sql);
        return $sql_result[$parameter];
}
