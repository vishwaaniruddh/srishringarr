...
///////////////rent qty
echo $sq21="SELECT  SUM(rent),sum(qty),sum(return_qty),sum(discount),SUM(CAST(REPLACE(commission_amt, ',', '') AS DECIMAL(10,2))) FROM  `order_detail` WHERE `bill_id` in (SELECT bill_id  FROM `phppos_rent` WHERE `cust_id`='$agent') and item_id ='$union[$i]'";
