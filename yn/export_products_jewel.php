...
if (isset($_REQUEST['pageno'])) {
    ...
    $total_pages = ceil($total_rows / $no_of_records_per_page);
        ...
        if($type=="2"){
            ...
            $sql_count = mysqli_query($con,"select count(product_id)  as count_a from `product` where subcat_id in(53,57,56,67,4,68,2,1,66,72,69,70,74,3,63,6,73,65,77,78,71,75,76,79,80,82
)");
            $sql_count = mysqli_fetch_assoc($sql_count);
            $product_count =  $sql_count['count_a'];
            ...
            youtube,CAST(REGEXP_SUBSTR(product_code,'[0-9]+') AS UNSIGNED) as sku,yn_productName from `product` where subcat_id in(53,57,56,67,4,68,2,1,66,72,69,70,74,3,63,6,73,65,77,78,71,75,76,79,80,82
) order by sku desc";
            $sql = mysqli_query($con,$statement);
        }
