...
if (isset($_REQUEST['pageno'])) {
    ...
    $total_pages = ceil($total_rows / $no_of_records_per_page);
            ...
            $sql_count = mysqli_query($con,"select count(product_for)  as count_a from `garment_product` where product_for in (10,22,28,29) and gproduct_id in(select gproduct_id from product_images_new where gproduct_id>0)");
            ...
            CAST(REGEXP_SUBSTR(gproduct_code,'[0-9]+') AS UNSIGNED) as sku,yn_productName from `garment_product` where product_for in(10,22,28,29) and gproduct_id
...
$product_desc = $row['gproduct_desc'];
    if($booking_status == 1){
            ...
            'link'=>$link,'cur_sql'=>$cur_sql,'imageframe'=>$imgframe,'qty'=>$qty,'discount_amount'=>$discount_amount,'product_desc'=>$product_desc];
    }else{
        ...
        'imageframe'=>$imgframe,'qty'=>$qty,'discount_amount'=>$discount_amount,'product_desc'=>$product_desc];
    }
} else if($type=="1"){
    ...
    'discount'=>$discount,'link'=>$link,'cur_sql'=>$cur_sql,'imageframe'=>$imgframe,'qty'=>$qty,'discount_amount'=>$discount_amount,'product_desc'=>$product_desc];
}
